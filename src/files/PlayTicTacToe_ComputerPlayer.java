...
public class PlayTicTacToe_ComputerPlayer {
	...
	public static void main(String args[]) throws Exception {
		...
		while (gameOver == false) {
			...
			if (turn % 2 == 0) {// if it is player 1's turn
				...
				do {
					// randomly choose an index to play
					index = (int) Math.floor(Math.random() * (9 - 1 + 1) + 1);// Assumes an integer is entered
					// check if you can play on that index and play
				...
				if (game.checkWin("O")) {
					...
					// end game when the computer wins
					gameOver = true;
				}
